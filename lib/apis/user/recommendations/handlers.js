...
module.exports = {
    getPersonalized: async (request, h) => {
        ...
        try {
            const { payload } = await Wreck.get(`/api/recommend/${userPassportNo}/${user.origin}`, { baseUrl });
    ...
    getPopular: async (request, h) => {
        const { user } = request.auth.credentials;
        try {
            const { payload } = await Wreck.get(`/api/popular/${user.origin}`, { baseUrl });
    ...
    getLatest: async (request, h) => {
        const { user } = request.auth.credentials;
        try {
            const { payload } = await Wreck.get(`/api/latest/${user.origin}`, { baseUrl });
            const recs = JSON.parse(payload).map(({ Id }) => Id);
            const articles = await getArticles({ request, recomendations: recs, user });
            return { articles };
        } catch (err) {
